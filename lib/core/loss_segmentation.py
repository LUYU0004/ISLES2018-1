...
class LossFunction(Layer):
    ...
    def layer_op(self,
        ...
        if weight_map is not None:
            ...
            # weight_map = tf.reshape(weight_map,
            # [tf.shape(weight_map)[0]*tf.shape(weight_map)[1]*tf.shape(weight_map)[2]*tf.shape(weight_map)[3]])
...
def dice(prediction, ground_truth, weight_map=None):
    ...
    # dice_score.set_shape([n_classes])
    # minimising (1 - dice_coefficients)
    print("returning both loss and score")
