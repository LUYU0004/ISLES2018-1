...
def group_norm(inputs,
  ...
  inputs_shape = tf.stack([tf.shape(inputs)[0]] + inputs_shape[1:])
  ...
  with variable_scope.variable_scope(
    ...
    org_shape = tf.stack([tf.shape(inputs)[0]] + list(original_shape)[1:])
    outputs = array_ops.reshape(outputs, org_shape)
