    ...
    def layer_op(self,
        ...
        with tf.device('/cpu:0'):
            ...
            if weight_map is not None:
                ...
                print("WeightMap Input ", weight_map)
...
def dice(prediction, ground_truth, weight_map=None):
    ...
    if weight_map is not None:
        ...
        intersection = one_hot * prediction
        square = tf.square(prediction)
        # union = square + one_hot
        dice_numerator = 2.0 * tf.sparse_reduce_sum(intersection, reduction_axes=[0])
        dice_denominator = tf.reduce_sum(square, reduction_indices=[0]) + \
                           tf.sparse_reduce_sum(one_hot, reduction_axes=[0])
